...
ACTIVE_DL  = -0.03
SCRIM_ALPHA = 0.72
SURFACES = ["panel", "item", "floating"]   # item … 面の上に載るもの（入力欄・入れ子の容れ物など）
JND = 0.02
...
def tokens(t, acc_name, acc_h):
    D = LADDERS[t["ladder"]]
    o = {}
    for k in ["bg","panel","item","floating","line-weak","line-strong","text-disabled","text-sub"]:
        o[k] = (D[k], t["cs"], t["h"])
...
def write_tables(data):
    ...
    L.append("### L 階段（2系統）\n")
    keys = ["bg","panel","item","floating","tint","line-weak","line-strong",
