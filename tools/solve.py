...
THEMES = [
    ...
    # ★2026-08-21 現在、この2枠は青系テーマの A/B 中（しろかさんが実物で判定する）。
    #   Mado の面（実測 H262 / C0.035〜0.063 の紺）に寄せたいという要求に対して、
    #   jelly を残せるかどうかで解が割れた。両方を出して見比べる。
    #     trench … Mado の色相そのまま。本物の紺だが jelly が ★ になる（8/9）
    #     fjord  … 9/9 を保てる最も青い色相。全色使えるが青紫寄りになる
    #   アクセント強さは両方 0.85 に揃えてある（差が色相と彩度だけになるように）。
